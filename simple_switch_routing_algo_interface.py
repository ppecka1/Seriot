...
# Piotr Pecka 
import simple_switch_conf as conf
...
from simple_switch_util import hid2mac # f. zamienia dpid na macc adres
...
class RoutingAlgoInterface(ABC):  
    def __init__(self,CAP,Tout,NS,f3d): # maksymalny rozmiar (CAPacity) kolekcji po ktorej wywolywany jest wybrany algorytm 
        ...
        """
        fcnt=0 #licznik flow: powinnno byc (NS-1)*(NS-1))
        for dpid_ in range(1,self.NS+1):
            fcnt=0
            for src_ in range(1,self.NS+1):
                for dst_ in range(1,self.NS+1):
                    if src_==dst_ or dpid_==dst_: continue
                    self.oldSettingsDim[dpid_][dst_][src_]=ports.getPort(dpid_,src_,dst_) # jestem w x - polacenie do y  ; z  -bierzacy odwiedzany wezel
                    fcnt+=1
            print("Number of flow settings for dpid=",dpid_,"  is equal ",fcnt," / expected:", (NS-1)*(NS-1))
        """
        ...
        for src_ in range(start,rang):
            for dst_ in range(start,self.NS+1): 
                ...
                if newPort!=self.oldSettingsDim[dpid][dst_][src_]: #and newPort!=-1 :
                    ...
                    mac_dst=hid2mac(dst_) # Atencjone: przy test band trzeba mapowac
                    mac_src=hid2mac(src_)
                    #hid2mac(3,4) nie dziala przeciazenie funcki
                    if self.f3d:
...
"""
    Algorytm Rnn
"""
from  simple_switch_rnnLib import RnnPathDesignator # parsuje info o portach (json) 
from  simple_switch_rnnLib import PayloadInfoSender # komunikuje sie z Rnn (wysyla info o obciazeniach i odbiera dane o portach (json))
...
class DWAlgo(RoutingAlgoInterface):
    def __init__(self,CAP,Tout=1,NS=12,topo=conf.TOPO):
        ...
        self.edgesCopy=[]
        self.sizeOfEdges=0
        self.ALFA=0.15 #(1-alfa)old+alfa(old)  wspolczynnik sredniej wazonej MUSI byc w przedziale  (0,1)
        try:  
