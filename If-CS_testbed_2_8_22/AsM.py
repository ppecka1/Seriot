...
def CogSender(dpid,log,startTime): # nadajnik pakietow Cog uzywa biblioteki  Process - nalezy przerobic na  thread aby mozna bylo uzyc wspolna pamiec 
    ...
    while True:
        while True: # wyslalem do wszystkich sasiadow 
            ...
            tim=time.time()
            msg = "%d, %d, %f, "% (dpid,tim,time.time()-startTime) #  trojka:  src,time[ns], startTime[s]  / dpid nadaje do x[0];   w x[]1] nr portu
            msg=bytes(msg,"ascii")
            #print ('wysylam')
            try:
...
if __name__ == '__main__': 
    ...
    pSender = Process(target=CogSender, args=(myId,log2,startTime,))
